...
from TOGL.topognn import Tasks
from TOGL.topognn.cli_utils import str2bool, int_or_none
from TOGL.topognn.layers import GCNLayer, GINLayer, GATLayer, SimpleSetTopoLayer, fake_persistence_computation#, EdgeDropout
from TOGL.topognn.data_utils import remove_duplicate_edges
from ph_cpu import compute_persistence_homology_batched_mt

import TOGL.topognn.coord_transforms as coord_transforms
import numpy as np
...
class TopologyLayer(torch.nn.Module):
    ...
    def compute_persistence(self, x,new_edge_index, vertex_slice, new_slices, batch, return_filtration = False):
        """
        ...
        vertex_slices = vertex_slice
        edge_slices = new_slices
    ...
    def forward(self, x, batch, return_filtration = False):
        ...
        new_edge_index, vertex_slice, new_slices, batches = remove_duplicate_edges(batch)

        persistences0, persistences1, filtration = self.compute_persistence(x, new_edge_index, vertex_slice, new_slices, batches, return_filtration)
        ...
        if self.dim1:
            ...
            graph_activations1 = self.collapse_dim1(coord_activations1, persistence1_mask, batch._slice_dict["edge_index"])  # returns a vector for each graph
        else:
