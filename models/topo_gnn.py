...
from models.gnn import EGNN_cont
import sys
sys.path.append('../')
...
class TopNN(nn.Module):
    def __init__(
        ...
        self.ODE_GNN = EGNN_cont(hidden_dim,num_node_features,global_pooling,n_steps,solver=solver)
        topo_layers = []
        self.ph_pooling_type = ph_pooling_type
    ...
    def forward(self, data):
        ...
        for i in range(ode_node_embed.shape[0]):
            ph_vectors += [self.ph_layers[i](self.node_dec(ode_node_embed[i]), data,pos)]
