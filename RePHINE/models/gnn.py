...
class EGNN_cont_v2(nn.Module):
    def __init__(
        ...
        self.embedding = torch.nn.Sequential(
            nn.Linear(num_node_features, hidden_dim//2),
            nn.ReLU(),
        ...
        self.egnn_model = EGNN(in_node_nf=hidden_dim+1,hidden_nf = hidden_dim,out_node_nf= hidden_dim,in_edge_nf=edge_nf,n_layers=2,residual=False,attention=True)
        self.edge_index = 0
        self.coords_weight=1.0
        self.edge_attr = 0
    ...
    def ode(self,t,data):
        ...
        tt = torch.ones_like(x[:, :1]) * t
        dx_final = torch.cat([tt.float(), x], 1)
        h,x = self.egnn_model(h=dx_final,x=pos,edges=self.edge_index, edge_attr=self.edge_attr)
        h = torch.cat([h,x],1)
        return h
    ...
    def forward(self, data):
        x, self.edge_index,pos,self.edge_attr = data.x, data.edge_index,data.pos,data.edge_attr
        time_steps = torch.linspace(0,1,steps=self.n_steps).to(data.x.device)
        ode_rhs  = lambda t,x: self.ode(t,x)
        x = self.embedding(x.float())
        x = torch.cat([x,pos],dim=1)
        x = odeint(ode_rhs,x,time_steps,method=self.solver,atol=1e-3,rtol=1e-3)
...
class EGNN_QM9(nn.Module):
    def __init__(
        ...
        self.egnn_model = EGNN(in_node_nf=hidden_dim+1, in_edge_nf=edge_nf, hidden_nf=hidden_dim, n_layers=1, coords_weight=1.0,attention=1, node_attr=0)
        self.embedding = torch.nn.Sequential(
            nn.Linear(num_node_features, hidden_dim//2),
            nn.ReLU(),
            nn.Linear(hidden_dim //2, hidden_dim))
    ...
    def ode(self,t,data):
        tt = torch.ones_like(data[:, :1]) * t
        dx_final = torch.cat([tt.float(), data], 1)
        h = self.egnn_model(h0=dx_final,x=self.pos,edges=self.edge_index, edge_attr=self.edge_attr,node_mask = self.node_mask, edge_mask=self.edge_mask, n_nodes=self.n_nodes)
        return h
    ...
    def forward(self, h0, x, edges, edge_attr, node_mask, edge_mask, n_nodes):
        ...
        time_steps = torch.linspace(0,1,steps=self.n_steps)
        ode_rhs  = lambda t,x: self.ode(t,x)
        h0 = self.embedding(h0.float())
        h0 = odeint(ode_rhs,h0,time_steps,method=self.solver,atol=1e-3,rtol=1e-3)
        return h0
