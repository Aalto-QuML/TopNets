...
class TopNN(nn.Module):
    ...
    def forward(self, data):
        ...
        for i in range(ode_node_embed.shape[0]):
            ...
            ph_vectors += [self.ph_layers[i](ode_node_embed[i], data,pos)]
...
class TopNN_QM9(nn.Module):
    def __init__(
        ...
        super().__init__()
        self.ODE_GNN = EGNN_QM9(hidden_dim,num_node_features,global_pooling,n_steps,solver=solver,edge_nf=edge_nf)
        topo_layers = []
        self.ph_pooling_type = ph_pooling_type
        for i in range(n_steps):
            topo = RephineLayer_Equiv(
    ...
    def forward(self,  h0, x, edges, edge_attr, node_mask, edge_mask, n_nodes,batch):
        ode_node_embed = self.ODE_GNN(h0, x, edges, edge_attr, node_mask, edge_mask, n_nodes)
        ph_vectors = []
        ode_node_embed_node  = ode_node_embed.view(self.num_steps,-1, n_nodes,self.hidden_nf)
        for batches in batch:
            for i in range(ode_node_embed.shape[0]):
                #temp_embed = self.node_dec(ode_node_embed[i])
                ph_vectors += [self.ph_layers[i](ode_node_embed[i], batches,x)]
        #for i, layer in enumerate(self.n_st):
        #    x = layer(x, edge_index=edge_index)
        #    ph_vectors += [self.ph_layers[i](x, data,pos)]

        # Pooling GNN embeddings
        x = torch.sum(self.node_readout(ode_node_embed_node[-1]), dim=1)
        # Pooling PH diagrams
        if self.ph_pooling_type == "mean":
            ph_embedding = torch.stack(ph_vectors).mean(dim=0)
        else:
