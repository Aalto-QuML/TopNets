...
from models.cell_mp import CochainMessagePassing, CochainMessagePassingParams
from torch_geometric.nn.inits import reset
from torch.nn import Linear, Sequential, BatchNorm1d as BN, Identity
from models.data_complex import Cochain
...
class SparseCINCochainConv(CochainMessagePassing):
    ...
    def forward(self, cochain: CochainMessagePassingParams):
        out_up, _, out_boundaries = self.propagate(cochain.up_index, cochain.down_index,
                                              cochain.boundary_index, x=cochain.x.float(),
                                              up_attr=cochain.kwargs['up_attr'],
                                              boundary_attr=cochain.kwargs['boundary_attr'])

        # As in GIN, we can learn an injective update function for each multi-set
        cochain.x = cochain.x.to(torch.float32)
        #print(out_up.dtype,cochain.x.dtype)
        out_up += (1 + self.eps1) * cochain.x
        cochain.x = cochain.x.to(torch.long)
        out_boundaries = out_boundaries.to(torch.long)
        self.eps2 = self.eps2.to(torch.long)
        #print(out_boundaries.dtype,cochain.x.dtype)
        out_boundaries += (1 + self.eps2) * cochain.x
        out_up = self.update_up_nn(out_up)
        out_boundaries = self.update_boundaries_nn(out_boundaries.to(torch.float32))
        #print(out_up.dtype,out_boundaries.dtype)
